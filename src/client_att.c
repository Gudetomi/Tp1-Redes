...
void Print_Carta2(Carta carta){
    printf("%c ",carta.valor);
    printf("%s\n", Define_Naipe(carta.naipe));
}
    ...
    while(1){
        ...
        if (strcmp(buffer, "Play") == 0){ // Deixa o cliente jogar
            ...
            if(qtd_mesa != 0){ // Se houver cartas na mesa
                ...
                // Print_Carta(mesa, qtd_mesa); // TODO: Mostrar as cartas pro cliente
                free(mesa);
                printf("\n\n");
            }else{
                printf("Mesa vazia!\n\n");
            ...
            if (strcmp(buffer, "Emp") != 0){
                printf("\n\nQuem ganhou a rodada foi o jogador %c, usando a carta: %c %s\n\n",buffer[0], buffer[1],Define_Naipe(buffer[2]));
            }else{
                printf("\n\nHouve Empate");
            }
